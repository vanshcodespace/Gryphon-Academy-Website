...
export default function Numbers() {
  ...
  return (
    <section className="relative overflow-hidden bg-[#fafafa] py-16 sm:py-20 lg:py-24">
      ...
      <div className="pointer-events-none absolute -left-40 top-0 h-[500px] w-[500px] rounded-full bg-[#cce0fc]/50 blur-[100px]" />
      <div className="pointer-events-none absolute -right-20 bottom-0 h-[400px] w-[400px] rounded-full bg-[#3a10b9ff]/10 blur-[100px]" />
      ...
      <div className="relative mx-auto max-w-7xl px-6 lg:px-8 xl:max-w-[85rem]">
        ...
        <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-2 lg:gap-16 xl:gap-20">
          ...
          <div className="relative flex min-h-[400px] flex-col items-center justify-center rounded-[2.5rem] border border-white bg-gradient-to-br from-[#ffffff] to-[#f4f7fc] p-10 shadow-[0_12px_40px_rgba(1,34,79,0.08)] backdrop-blur-xl lg:min-h-[500px]">
            <img 
              src={mapIcon} 
          ...
          <div className="grid grid-cols-2 text-center">
            ...
            <div className="flex flex-col items-center justify-center border-b-[2.5px] border-r-[2.5px] border-dashed border-[#01224F] p-4 sm:p-8 md:p-10">
              <span className="text-[2.5rem] font-extrabold leading-tight text-[#01224F] md:text-5xl lg:text-[3.25rem]">{stats[0].number}</span>
              <span className="mt-3 whitespace-pre-line text-sm font-medium text-[#01224F] md:text-lg lg:text-xl">{stats[0].label}</span>
            </div>
            
            {/* Top Right */}
            <div className="flex flex-col items-center justify-center border-b-[2.5px] border-dashed border-[#01224F] p-4 sm:p-8 md:p-10">
              <span className="text-[2.5rem] font-extrabold leading-tight text-[#01224F] md:text-5xl lg:text-[3.25rem]">{stats[1].number}</span>
              <span className="mt-3 whitespace-pre-line text-sm font-medium text-[#01224F] md:text-lg lg:text-xl">{stats[1].label}</span>
            </div>

            {/* Bottom Left */}
            <div className="flex flex-col items-center justify-center border-r-[2.5px] border-dashed border-[#01224F] p-4 sm:p-8 md:p-10">
              <span className="text-[2.5rem] font-extrabold leading-tight text-[#01224F] md:text-5xl lg:text-[3.25rem]">{stats[2].number}</span>
              <span className="mt-3 whitespace-pre-line text-sm font-medium text-[#01224F] md:text-lg lg:text-xl">{stats[2].label}</span>
            </div>

            {/* Bottom Right */}
            <div className="flex flex-col items-center justify-center p-4 sm:p-8 md:p-10">
              <span className="text-[2.5rem] font-extrabold leading-tight text-[#01224F] md:text-5xl lg:text-[3.25rem]">{stats[3].number}</span>
              <span className="mt-3 whitespace-pre-line text-sm font-medium text-[#01224F] md:text-lg lg:text-xl">{stats[3].label}</span>
            </div>
          </div>
